...
void executeOpenCL(const std::string& kernelName, const std::string& kernelSource, const unsigned char *dataInput, 
  const int dataInputSize, unsigned char *dataOutput, const int dataOutputSize) {
  ...
  cl::Buffer outputBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR, sizeof(unsigned char) * dataOutputSize, nullptr);
  
  cl::CommandQueue queue(context, defaultDevice);
  queue.enqueueWriteBuffer(
    inputBuffer, CL_TRUE, 0, sizeof(unsigned char) * dataInputSize, dataInput);
